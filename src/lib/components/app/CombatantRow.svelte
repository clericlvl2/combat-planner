...
<script lang="ts">
	...
	const tagTriggerClass =
		'inline-flex h-[22px] items-center gap-[5px] rounded-full border border-dashed border-border px-2.5 py-0.5 text-sm text-muted-foreground transition-[transform,border-color,color] duration-[var(--dur-base)] ease-[var(--ease-out)] hover:border-foreground hover:text-foreground active:translate-y-px';
		...
		<div class="min-w-0 flex-1 p-[var(--card-pad)]">
			...
			>
				<div class="flex flex-col">
					...
					<button
						type="button"
						class="flex min-h-10 w-full cursor-pointer items-center gap-3 rounded-md px-1 py-1 transition-[transform,background-color] duration-[var(--dur-base)] ease-[var(--ease-out)] hover:bg-secondary active:translate-y-px lg:min-h-8"
						aria-label={hpLabel}
