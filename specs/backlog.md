...
| W-049 | S | small | inbox | Boot-shell robustness, from the W-041 post-ship review: `#boot-skeleton` hardcodes `52px` against `AppHeader`'s rem-based `h-13` (drifts at a non-16px root font — belongs in `tokens.css` as a shared `--header-h`); `NavSidebar`'s `loadDrawer()` has no `.catch`, so a failed chunk fetch is an unhandled rejection that can leave `open = true` with nothing mounted; the 15s failure timer should cancel on first evidence of life rather than racing a genuinely slow device into a hardcoded-English reload prompt | — | — |
