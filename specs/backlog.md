...
| W-032 | M | large | inbox | App composition layer — `ResponsiveModal` owning the Dialog/Drawer split, scroll region, sticky footer, safe-area padding, sizing and drawer touch policy; migrate all 6 modal call sites; extract `Field`/`TypeToggle`; delete dead vendoring (`sheet`/`progress`/`separator`); ADR-014 | — | `.claude/plans/2026-07-26-responsive-modal-app-layer.md` |
