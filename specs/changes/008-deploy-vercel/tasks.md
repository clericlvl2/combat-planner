...
- [ ] Run `npm run build`; confirm it emits `build/index.html` (the adapter-static SPA fallback
...
- [ ] Connect `clericlvl2/combat-planner` to Vercel via Git integration; set Production Branch =
      `main`. Confirm `vercel.json` is picked up (no framework preset override).
- [ ] Trigger a production deploy (merge/push to `main`); confirm a new deployment appears and
      succeeds. (AC: PLT-9 push-to-main auto-deploys)
- [ ] `curl -I <prod-url>` returns `200` over HTTPS and the body is the app shell HTML.
      (AC: PLT-9 reachable)
- [ ] Record the production URL + deploy dashboard link in `deploy-notes.md` so spec-verifier can
