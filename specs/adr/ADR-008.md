...
**Rationale.** shadcn-svelte gives copy-in, ownable, accessible components (built on Bits UI) rather than a heavy locked component library — good for a small bundle and for the bespoke combatant row / numpad / drag list. Tailwind enables the dark+light theming and high-contrast tokens (incl. the reverse/alarm HP bar, the HP-status card-background tints, and the three fixed combatant-type colors: PC green, enemy red, ally blue — reassigned from an earlier PC-blue/ally-green pairing during the first-touch rework) with consistent spacing and ≥44px touch targets.
