...
| W-046 | M | large | active | Defer `CombatFormDialog` / `ConfirmDialog` / `CombatRowMenu` inside the route components — this is where bits-ui and vaul actually enter `/` and `/combats` (measured: 54-56 KB gz off `/`'s 160,557 gz route closure, more than the row first estimated). W-041 Phase 5 took the layout-level win only (sonner + NavSidebar's Drawer, −10.8 KB gzip, render-blocking stylesheets 3→1); these three sit in route components outside that phase's scope. Needs each dialog behind its own action-gated dynamic import, plus a re-measure | — | `.claude/plans/2026-07-30-defer-route-dialogs.md` |
